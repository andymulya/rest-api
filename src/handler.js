...
const getAllBooksHandler = (request, h) => {
  ...
  const books = detailBooks.map((book) => ({ id: book.id, name: book.name, publisher: book.publisher }));

  if (name !== undefined) {
    const lb = detailBooks.filter((book) => book.name.toLowerCase().includes(name.toLowerCase()) !== false);
    const book = lb.map((listBook) => ({
      id: listBook.id,
      name: listBook.name,
      publisher: listBook.publisher,
    }));
    return query(book, h);
  }
